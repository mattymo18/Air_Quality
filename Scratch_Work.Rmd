...
```{r}
# DF.Green$Stay_At_Home <- factor(DF.Green$Stay_At_Home)
# DF.Green$Bar_Close <- factor(DF.Green$Bar_Close)
set.seed = 18
trainIndex <- createDataPartition(y = DF.Green$newcasesper, p=.8, times = 1, list = F)
train_ctrl <- trainControl(method = "repeatedcv", number = 50)
DF.Green.Test <- DF.Green[-trainIndex, ]

glm.test <- train(newcasesper ~ O3 + 
                    PM25 + 
                    Stay_At_Home +
                    Bar_Close, 
...
preds <- predict(glm.test, newdata = DF.Green.Test, type = "raw")
Results <- eval_results(DF.Green.Test$newcasesper, preds, DF.Green.Test)


glm1 <- glm(newcasesper ~ O3 + 
                    PM25 + 
                    Stay_At_Home +
                    Bar_Close, 
                  data = DF.Green[trainIndex, ], 
