...
Columbia_Air <- read.csv("Source_Data/Columbi_Air.csv") %>% 
  select(city, utc, parameter, value, latitude, longitude)
```
...
Columbia <- spread(Columbia_Air, key = parameter, value = value)

Columbia$utc = as.Date(Columbia$utc) 
Columbia$Time <- format(Columbia$utc,"%H:%M:%S")
Columbia = Columbia %>% 
  select(-Time)
Columbia.Clean <- Columbia %>% 
  group_by(utc, .drop = T) %>% 
  summarise(CO = mean(co), 
            O3 = mean(o3), 
            PM25 = mean(pm25), 
            SO2 = mean(so2)) %>% 
  mutate(Location = "Columbia") %>% 
  arrange(utc) %>% 
  select(Location, utc, CO, O3, PM25, SO2)
```
